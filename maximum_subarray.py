...
class Solution:
    def maxSubArray(self, nums: List[int]) -> int:
        ...
        # taking sum as first value in case of single item into the list
        sum = nums[0]
        
        # Looping into the list except 1st element as we have taken the same earlier for initialization.
        for n in nums[1:]:
        
            # checking if the next item is greater then sum if yes then will set sum as thet itmem's value
            if n > sum + n:
                sum = n
